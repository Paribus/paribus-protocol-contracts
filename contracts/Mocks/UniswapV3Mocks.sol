...
contract UniswapV3SwapRouterMock is IUniswapV3SwapRouter {
    ...
    function exactInputSingle(ExactInputSingleParams calldata params) external payable returns (uint256) {
        ...
        EIP20Interface(params.tokenIn).transferFrom(msg.sender, address(this), params.amountIn);
        EIP20Interface(params.tokenOut).transfer(params.recipient, params.amountOutMinimum + extra);
    ...
    function exactOutputSingle(ExactOutputSingleParams calldata params) external payable returns (uint256) {
        EIP20Interface(params.tokenIn).transferFrom(msg.sender, address(this), params.amountInMaximum - 100);
        EIP20Interface(params.tokenOut).transfer(params.recipient, params.amountOut);
