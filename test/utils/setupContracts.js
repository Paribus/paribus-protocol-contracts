...
const setupCryptoPunksMock = async function () {
    const CryptoPunksMock = await ethers.getContractFactory("CryptoPunksMarketMock")
    const cryptoPunksMock = await CryptoPunksMock.deploy()
    await cryptoPunksMock.allInitialOwnersAssigned()
    return cryptoPunksMock
}

const setupAllTokens = async function () {
    const [owner] = await ethers.getSigners()

    const [dai, wbtc, usdc, weth, pbx, sada, erc721, cryptopunks] = await Promise.all([
            setupToken('Dai', 18, '10000000', '0', [(await ethers.provider.getNetwork()).chainId]),
            setupToken('WBTC', 8, '10000000', '0', []),
            setupToken('FiatTokenV2', 6, '10000000', '0', [], async (token) => {
            ...
            setupToken('PBXTokenMock', 18, '0', '0', [tokens('10000000000000')]),
...
const setupAllPTokens = async function (tokensContracts, platformContracts) {
    const [pdai, pwbtc, pusdc, ppbx, pweth, peth, psada] = await Promise.all([
        ...
        setupPEther('pEther', 'pETH', 8, '200000000000000000000000000', platformContracts.unitroller, platformContracts.jrmEth),
...
const setComptrollerImpl = async function (unitrollerAddress, newComptrollerPart1, newComptrollerPart2) {
    ...
    await unitroller._setPendingImplementations(newComptrollerPart1.address, newComptrollerPart2.address)
    await Promise.all([
        newComptrollerPart1._become(unitroller.address),
        newComptrollerPart2._become(unitroller.address),
    ])
}
