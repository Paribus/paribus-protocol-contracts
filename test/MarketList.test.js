...
describe("Compound Market list", () => {
    beforeEach(async () => {
        app = await setupAll()
        await app.weth.connect(app.user1).deposit({value: tokens('5')})
    })
    ...
    it("getAssetsIn", async () => {
        ...
        await app.weth.connect(app.user1).approve(app.pweth.address, tokens('9999999999'))
